...
calc_segment_base: ; 计算 16 位段地址
	...
	; dx=0x0000
	; ax=0x0020
	; 尽管 DX:AX 中是 32 位的用户程序起始物理地址，理论上，它只有 20 位是有效的，低 16 位在寄存器 AX 中，高 4 位在寄存器 DX 的低 4 位。
	; 寄存器 AX 经右移后，高 4 位已经突出，只要将 DX 的最低 4 位挪到这里，就可以得到我们所需要的逻辑段地址。
	add ax,[cs:phy_base]
